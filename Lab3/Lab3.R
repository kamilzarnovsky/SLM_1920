...
#Function making variable histograms + density estimation
...
#A function that performs point plots with linear regression and spline
DoScatterplot = function(x.values, x.name) {
  plot(x.values, DATA_SET$MORT,
       xlab = x.name, ylab = "MORT", main = NULL, col = gray(0.4))
  lines(smooth.spline(x.values, DATA_SET$MORT), lwd = 2) #smoothing spline
  abline(lm(DATA_SET$MORT ~ x.values)$coef, col = gray(0.7), lwd = 2, lty = 2) #linear regression
}
#Boxplots plotting
...
invisible(mapply(DoBoxplot, DATA_SET, names(DATA_SET))) #invisible - Return a (temporarily) invisible copy of an object.
cat("\nCorrelation matrix:\n")
cormat = cor(DATA_SET)
...
plot.new()
plot.new()
legend("left", c("data", "line", "smoothed"), bty = "n",
...
# The "span" parameter controls how many observations are considered as a neighbor of the analyzed point
fit = loess(MEDV~LSTAT, span=.2, data=MERGED) # s=0.2 (better local fit, curve less smooth)
fit2 = loess(MEDV~LSTAT, span=.5, data=MERGED) # s=0.5 (better global fit, curve more smooth)
